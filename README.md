

# B Vighnesh Kumar

Backend Developer
Java <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/java/java-original.svg" height="18"/>  
Spring Boot <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/spring/spring-original.svg" height="18"/>  
MySQL <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/mysql/mysql-original.svg" height="18"/>  
React.js <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/react/react-original.svg" height="18"/>  

---

## About Me

Backend-focused developer with full stack exposure, experienced in building REST APIs and end-to-end applications.

* Strong in backend development and system design
* Experience with authentication, database design, and API integrations
* Solid foundation in Data Structures, OOP, DBMS, Operating Systems, and Computer Networks
* Currently pursuing B.E. in Computer Science (CGPA: 8.26)
* Seeking Software Engineer and Backend Developer roles

---

## Technical Skills

### Languages
<p>
  <img src="https://img.shields.io/badge/Java-ED8B00?style=flat&logo=java&logoColor=white"/>
  <img src="https://img.shields.io/badge/SQL-003B57?style=flat&logo=mysql&logoColor=white"/>
  <img src="https://img.shields.io/badge/JavaScript-F7DF1E?style=flat&logo=javascript&logoColor=black"/>
  <img src="https://img.shields.io/badge/C-00599C?style=flat&logo=c&logoColor=white"/>
</p>

### Backend
<p>
  <img src="https://img.shields.io/badge/Spring_Boot-6DB33F?style=flat&logo=spring-boot&logoColor=white"/>
  <img src="https://img.shields.io/badge/REST_API-000000?style=flat"/>
  <img src="https://img.shields.io/badge/MVC-1E90FF?style=flat"/>
</p>

### Frontend
<p>
  <img src="https://img.shields.io/badge/React-20232A?style=flat&logo=react&logoColor=61DAFB"/>
  <img src="https://img.shields.io/badge/HTML-E34F26?style=flat&logo=html5&logoColor=white"/>
  <img src="https://img.shields.io/badge/CSS-1572B6?style=flat&logo=css3&logoColor=white"/>
  <img src="https://img.shields.io/badge/JavaScript-F7DF1E?style=flat&logo=javascript&logoColor=black"/>
</p>

### Database
<p>
  <img src="https://img.shields.io/badge/MySQL-00758F?style=flat&logo=mysql&logoColor=white"/>
  <img src="https://img.shields.io/badge/JDBC-000000?style=flat"/>
  <img src="https://img.shields.io/badge/Relational_DB-4B8BBE?style=flat"/>
</p>

### Tools
<p>
  <img src="https://img.shields.io/badge/Git-F05032?style=flat&logo=git&logoColor=white"/>
  <img src="https://img.shields.io/badge/IntelliJ_IDEA-000000?style=flat&logo=intellij-idea&logoColor=white"/>
  <img src="https://img.shields.io/badge/Postman-FF6C37?style=flat&logo=postman&logoColor=white"/>
</p>

---

## Projects

### AagriGgate — Agricultural Marketplace Platform

[https://github.com/B-Vighnesh/AagriGgate](https://github.com/B-Vighnesh/AagriGgate)

* Built a full stack platform connecting farmers directly with buyers
* Designed and developed REST APIs using Spring Boot
* Developed frontend using React.js
* Integrated MySQL database
* Implemented role-based access control

---

### Dark Pattern Detector — ML Integrated System

[https://github.com/B-Vighnesh/Dark-Pattern-Detector](https://github.com/B-Vighnesh/Dark-Pattern-Detector)

* Developed a system to detect deceptive UI patterns
* Integrated DistilBERT model with a browser extension
* Built backend using Spring Boot
* Developed frontend using React.js
* Enabled real-time detection

---

### EquiManage — Equipment Management System

[https://github.com/B-Vighnesh/EquiManage](https://github.com/B-Vighnesh/EquiManage)

* Developed a Java-based equipment tracking system
* Designed MySQL database schema
* Implemented CRUD operations using JDBC
* Improved inventory management process

---

## Experience

Software Product Developer (Apprenticeship)
Sclera VDMS | Feb 2026 – Present

* Learning backend development and product workflows
* Understanding end-to-end system architecture
* Studying data flow between frontend, backend, and database

---

## Education

Bachelor of Engineering in Computer Science and Engineering
Canara Engineering College (Expected 2026)
CGPA: 8.26

---

## Certifications

* Programming in Java — IIT Kharagpur (NPTEL)
* Java Programming — Duke University (Coursera)
* Introduction to Databases — Meta (Coursera)

---
## Contact

Email: [bvighneshkumar@gmail.com](mailto:bvighneshkumar@gmail.com)  
LinkedIn: https://linkedin.com/in/b-vighnesh-kumar  
GitHub: https://github.com/B-Vighnesh  
LeetCode: https://leetcode.com/u/B_Vighnesh_Kumar  

Portfolio:  
https://b-vighnesh-portfolio.vercel.app

---
## GitHub Stats

<p align="center">
  <img src="https://github-readme-stats.vercel.app/api?username=B-Vighnesh&show_icons=true&theme=tokyonight&hide_border=true" />
  <img src="https://github-readme-stats.vercel.app/api/top-langs/?username=B-Vighnesh&layout=compact&theme=tokyonight&hide_border=true" />
  
</p>
<p align="center">
  <img src="https://streak-stats.demolab.com?user=B-Vighnesh&theme=tokyonight&hide_border=true" />
</p>
